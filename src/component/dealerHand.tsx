				...
				<div id={role} className="flex justify-center mt-14 relative scale-[0.8] flex-wrap w-[300px]">
						...
						{deck.map((card, index) => {
							if (canPlay) {
								...
								return <div key={index} className={`flex justify-center items-center text-4xl text-[#6D5C5C]  bg-cover w-24 h-36 -ml-6 ${hidden}`}>
									{index != 0 ? card.figure : ''}
								</div>
							} else {
								const sendToAbove = index > 2 ? `absolute left-${index > 3 ? 12 : 6} top-28` : '';
								const animateNewCard = index > 1 ? 'animate-getCard' : '';
								return <div key={index} className={`flex justify-center items-center text-4xl text-[#6D5C5C] bg-[url('/src/assets/cards/card.png')] bg-cover w-24 h-36 -ml-6 ${animateNewCard} ${sendToAbove}`}>
									{card.figure}
								</div>
							}
						})}
